...
$stmt = $conn->prepare("INSERT INTO vr20_studylog (user_id, course, activity, time) values (?,?,?,?)");
echo $conn->error;
//seon päringuga tegelikud andmed
$user_id = $_SESSION["userid"];
$stmt->bind_param('iiid', $user_id, $course, $activity, $time);
//i-integer; s-string
if ($stmt->execute()) {
$response = 1;
...
function showLog(){
    ...
    FROM vr20_studylog 
    LEFT JOIN vr20_users ON vr20_studylog.user_id=vr20_users.id 
    ...
    GROUP BY vr20_studylog.user_id, vr20_studylog_course.course_name, vr20_studylog_activity.course_activity
    ORDER BY vr20_studylog.user_id, vr20_studylog_course.course_name");



    echo $conn->error;
    $stmt->bind_result($firstFromDB, $lastFromDB, $activityNameFromDB, $timeFromDB, $CourseNameFromDB);
    $stmt->execute();
 
    $response .="<table border=1><tr><td>Õppeaine</td><td>Tegevus</td><td>Aeg(h)</td><td>Kasutaja</td></tr>\n";

    while ($stmt->fetch()){
        $response .="<tr><td>" .$CourseNameFromDB." </td><td> ".$activityNameFromDB. "</td><td>" .$timeFromDB. "</td><td>" .$firstFromDB. " ".$lastFromDB. "</td></tr>\n";
        if ($response == null){
